...
knitr::opts_chunk$set(
	...
	out.width = "100%",
	results = "hold",
	tidy.opts=list(width.cutoff = 115),
	tidy = TRUE
)
```

# Disclaimer
|               This notebook is designed to explore the molecular data published at DOI: 10.1128/iai.00263-23; This notebook is free to use under the terms of the MIT license on GitHub; you can redistribute it and/or modify it under the terms of the GNU General Public License, version 3, as published by the Free Software Foundation. This notebook is distributed in the hope that it will serve educational and entertainment purposes, but without any warranty; without even the implied warranty of merchantability or fitness for a particular purpose. Use of this code may be subjected to the licensing agreement and copyright requirement of the ASM Journal, Infection and Immunity, where the raw data is published.
|               For more detail of the GNU license, please refer to: www.gnu.org/licenses/gpl-3.0.en.html; for more detail of the GitHub MIT license, please refer to: https://opensource.org/license/mit; for more detail of the Creative Comment CC-BY 4.0, please refer to: https://creativecommons.org/licenses/by/4.0/; for more detail of ASM journal licensing agreements, please refer to (1) license in use section and (2) author warranty and provosional license to publish section at: https://journals.asm.org/licensing-copyright.
...
# Data source
|               Article citation: Koyuncu D, Tavolara T, Gatti DM, Gower AC et al. B cells in perivascular and peribronchiolar granuloma-associated lymphoid tissue and B-cell signatures identify asymptomatic $\tkxtlf{Mycobacterium tuberculosis}$ lung infection in Diversity Outbred mice. Infect Immun 2024 Jul 11;92(7):e0026323. PMID: 38899881.
|               Raw expression microarray data is imported from Gene Expression Omnibus (GEO): https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE266564. Gene Ontology (GO) data is obtained from the associated platform, namely GPL16570, and the R package, org.Mm.eg.db. GPL32068 is the annotation platform published in the source study, and GPL16570 is described as identical to GPL32068, except that a custom CDF environment was used to extract data [Brainarray Version 24, mogene20st_Mm_ENTREZG]. Annotation on GPL1650 is used in this study because it has more comprehensive data (e.g., both protein domain and biological processes) and the table structure is closer to the standard used on GEO.

# Context
|               The source article experimented on lung samples of Diversity Outbred mice infected by $\textit{Mycobacterium tuberculosis}$ (TB) over maximally 60 days. Three states of infections were identified and classified into groups, namely not infected, asymptomatic controller, chronic controller, and progressor groups with definitions as follows: (1) uninfected mice were not experimentally infected by TB; (2) asymptomatic mice were experimentally infected but lacked diagnosible symptom, which were euthanised at predetermined date at of before 60 days post-infection; (3) chronically infected mice with symptoms of TB, which succumbed to TB and were euthanized after 60 days post-infection; (4) progressively infected mice with with symptoms of TB, which morbidly succumbed to TB and were euthanized before 60 days post-infection.
...
## Create four sets of Gene Ontology (GO) terms and Protein Domain (PD)
### Cleaning the gene, GO, and PD data
...
# parsing protein domain data, as the custom cdf environment in the original study uses protein domain data.
parsPD <- function(probe, protein_domain = "protein_domains"){
  pd_segment <- strsplit(GPL16570_lung[probe, "protein_domains"], "///")[[1]]
  entrez_id <- trimws(unlist(lapply(pd_segment, function(x) strsplit(x, "//")[[1]][1])))
  inter_pro_id <- trimws(unlist(lapply(pd_segment, function(x) strsplit(x, "//")[[1]][3])))
  protein_segment <- trimws(unlist(lapply(pd_segment, function(x) strsplit(x, "//")[[1]][4])))
  protein <- trimws(unlist(lapply(protein_segment, function(x) strsplit(x, "/")[1])))
  
  # in case of probe without id (a.k.a. errors in data wrangling)
  if (length(entrez_id) > 0){
    pd_tab <- data.frame(
      Probe_id = rep(probe, length(protein)),
      entrez_id = rep(entrez_id, length(protein)),
      inter_pro_id = rep(inter_pro_id, length(protein)),
      protein = protein)
    #rownames(pd_tab) <- rep(probe, nrow(pd_tab))
    return(pd_tab)
    return() # return nothing if pd_family is empty/NA
  }
}

# arrange the APD and PD data
prepPD <- function(degs){
  # match information between Array PD terms from DEG result and GPL16750_lung.
  APD <- cbind(Gene_Symbol = GPL16570_lung[rownames(degs), c("probeset_id", "Gene_id", "Gene_title")],
               degs[, c("logFC", "P.Value", "adj.P.Val")])
  colnames(APD) <- c("Probe_id", "Gene_id", "Gene_title", "logFC", "P.Val", "adj.P.Val")
  
  # parse all PD terms on protein domain column from GPL16570. 
  PD <- c()
  
  for (probe in rownames(APD)){
    PD <- rbind(PD, parsPD(probe))}
  
  # drop the rows with only na.
  PD <- as.data.frame(PD[rowSums(is.na(PD)) != ncol(PD), ])
  PD <- na.omit(PD)
  
  # match APD and PD, as DEGs without PD term is of little significant to our purpose.
  APD <- APD[which(APD$Probe_id %in% PD$Probe_id), ]
  APD <-na.omit(APD)
  
  # combine the result into a list for subsequent enrichment analysis.
  result <- list(APD, PD)
  names(result) <- c("APD", "PD")
...
# can not use prepPD and prepGO due to inconsistent column names. We could rename columns but it may cause issues if not careful.
degs <- probe_results_all

# parse the PD terms.
# match information between Array PD terms from DEG result and GPL16750_lung.
APD <- cbind(Gene_Symbol = GPL16570_lung[rownames(degs), c("probeset_id", "Gene_id", "Gene_title")],
               degs[, c("F", "P.Value", "adj.P.Val")])
colnames(APD) <- c("Probe_id", "Gene_id", "Gene_title", "F", "P.Val", "adj.P.Val")
...
Genepd_four <- list(APD, PD)
names(Genepd_four) <- c("APD", "PD")

# parse the GO terms.
# match information between Array GO terms from DEG result and GPL16750_lung.
AGO <- cbind(Gene_Symbol = GPL16570_lung[rownames(degs), c("probeset_id", "Gene_id", "Gene_title")],
             degs[, c("F", "P.Value", "adj.P.Val")])
colnames(AGO) <- c("Probe_id", "Gene_id", "Gene_title", "F", "P.Val", "adj.P.Val")
AGO <- na.omit(AGO)
...
# combine the result into a list for subsequent enrichment analysis.
Genego_four <- list(AGO, GO)
names(Genego_four) <- c("AGO", "GO")
```
...
# parse gene IDs and PD terms.
Genepd_ui_ac <- prepPD(degs)
head(Genepd_ui_ac$APD)
head(Genepd_ui_ac$PD)
...
# parse gene IDs and PD terms.
Genepd_ui_cc <- prepPD(degs)
head(Genepd_ui_cc$APD)
head(Genepd_ui_cc$PD)
...
# parse gene IDs and PD terms.
Genepd_ui_pr <- prepPD(degs)
head(Genepd_ui_pr$APD)
head(Genepd_ui_pr$PD)
...
# parse gene IDs and PD terms.
Genepd_ac_cc <- prepPD(degs)
head(Genepd_ac_cc$APD)
head(Genepd_ac_cc$PD)
...
# parse gene IDs and PD terms.
Genepd_ac_pr <- prepPD(degs)
head(Genepd_ac_pr$APD)
head(Genepd_ac_pr$PD)
...
# parse gene IDs and PD terms.
Genepd_cc_pr <- prepPD(degs)
head(Genepd_cc_pr$APD)
head(Genepd_cc_pr$PD)
...
### Anlysis on protein domain (attemp to recreate the result of the original study)
#### Across four groups
```{r enrichment using enricher from clusterprofiler}
# get data from deg results and from annotation data
deg <- Genepd_four$APD
annotation <- Genepd_four$PD

# combine deg and annotation based on Probe_id
deg_merge <- merge(deg, annotation, by = "Probe_id")
deg_merge <- deg_merge %>%
  select(Probe_id, entrez_id, protein)

# create a dataframe fo map gene and protein family
gene_to_pd <- data.frame(
  protein = deg_merge$protein,
  entrez_id = deg_merge$entrez_id)
gene_to_pd <- gene_to_pd[!duplicated(gene_to_pd), ]

# create background count
bg_gene <- unique(deg_merge$Gene_id)

# perform enrichment analysis.
EA_PD_four <- enricher(gene = deg$Gene_id, 
                       pvalueCutoff = 0.05,
                       pAdjustMethod = "BH",
                       TERM2GENE = gene_to_pd, 
                       universe = bg_gene,
                       minGSSize = 1,
                       maxGSSize = 500)
# view the result as table
head(EA_PD_four)
```

##### Uninfected vs Aymptomatic Controller
```{r define a function for manual enrich computation}
# you can also use enricher() from the clusterProfiler above. 
# We can also use the hypergeometric test to manually compute for significant protein domains.
EAPDCompute <- function(apd, pd){
  # assign the arguments.
  APD <-  apd
  PDdata <- pd
  
  # check the minimal adj.P.Val.
  if (min(APD$adj.P.Val) < 0.05){
    # perform the EA computation.
    # compute for foreground
    fg_probe <- rownames(APD)[which(APD[, "adj.P.Val"] < 0.05)]
    fg_pd_id <- PDdata$Probe_id[PDdata$Probe_id %in% fg_probe]
    
    # compute for background
    bg_probe <- rownames(APD)
    bg_pd_id <- PDdata$Probe_id[PDdata$Probe_id %in% bg_probe]
    
    # Initialize an empty matrix to store the results
    EA <- data.frame()
    
    # counting for each protein term as if they were GO terms
    for (Id in unique(fg_pd_id)) {
      # tocal count in background with PD term
      m <- sum(bg_pd_id == Id) 
      # total count in background without PD term
      n <- length(bg_pd_id) - m 
      # total count in foreground
      q <- length(fg_pd_id)
      # total count in foreground with PD term
      x <- sum(fg_pd_id == Id)
    ...
    rownames(EA) <- unique(fg_pd_id)
    colnames(EA) <- c("m", "o(m + n)", "q", "x", "P.Val.hygeo")
    
    if(nrow(EA) > 0 && any(!is.na(EA[, "x"]))){
      # adjust the P.Val.hygeo for multiple testing across the entire array of probes. 
      # Finding true positive is more important to me in this analysis, but use one of the other adjusting methods as needed.
      ...
      PDsig <- rownames(EA)[which(EA[, "adj.P.Val.hygeo"] < 0.05)]
      
      # grab and add GO data.
      PDname <- c()
      
      for (pd in PDsig) {
        pd_des <- unique(PDdata[which(PDdata[, "Probe_id"] == pd), "protein"])
        
        # if no pd_des is found, append NA 
        if (length(pd_des) == 0) {
          PDname <- c(PDname, NA)
          unmatched_count <- unmatched_count + 1
        } else {
          # append the first pd_des if duplicates are found
          PDname <- c(PDname, pd_des[1])
        }
      }
      
      EA <- cbind(Description = PDname, EA[PDsig, , drop = FALSE]) # ensure dataframe structure not changed.
      
      # we can format the enrichment analysis output as observed and expected, which may be easier to interpret.
      # recall that we want to compare the foreground and background ratios. DEG with the selected protein domain count.
      # foreground ratio.
      ...
      EA <- data.frame(cbind(`Probe_id` = rownames(EA),
                             Description = EA[, "Description"],
                             `Expected Frequency` = paste0(Ratio_mu_bg, "/", EA[, "q"]),
                             `Observed Frequency` = paste0(EA[, "m"], "/", EA[, "q"]),
                             `Background Ratio` = paste0(EA[, "m"], "/", EA[, "o(m + n)"]),
                             `FoldChange` = round(Ratio_fg/Ratio_bg, 5), # of protein domain.
                             `P.Val` = as.numeric(EA[, "P.Val.hygeo"]),
                             `P.Val.adjusted` = as.numeric(EA[, "adj.P.Val.hygeo"])))
      
      # add gene information
      join_apd <- APD
      join_apd$Probe_id <- as.character(join_apd$Probe_id)
      
      EA_join <- EA %>%
        left_join(join_apd, by = "Probe_id")
      
      # remove duplicates and unnecessary columns
      EA_join <- select(EA_join, c("Probe_id", "Gene_id", "logFC", "Description", "P.Val.x", "P.Val.adjusted", 
                                   "Expected.Frequency", "Observed.Frequency", "Background.Ratio"))
      
      colnames(EA_join) <- c("Probe_id", "Gene_id", "logFC", "Description", "P.Val", "P.Val.adjusted", 
                             "Expected.Frequency", "Observed.Frequency", "Background.Ratio")
      
      EA_join <- unique(EA_join)
      # remove duplicate for a clean output table.
      tempt <- EA %>%
        select(c("Probe_id", "Description", "Expected.Frequency", "Observed.Frequency", 
                 "Background.Ratio", "FoldChange", "P.Val", "P.Val.adjusted"))
      
      join_list <- EA_join %>%
        select(c("Probe_id", "Gene_id"))
      
      tempt <- tempt %>%
        left_join(join_list, by = "Probe_id") 
      ...
      EA_tab <- tempt %>%
        left_join(EA, by = "Description")
      
      EA_tab <- EA_tab %>%
        select("Probe_id", "Description", "Gene.id", "Expected.Frequency", "Observed.Frequency",
               "Background.Ratio", "FoldChange", "P.Val", "P.Val.adjusted")
      
      Protein_tab <- unique(EA_tab[, c("Description", "Gene.id", "P.Val.adjusted")])
      Top_protein <- Protein_tab %>%
        arrange(P.Val.adjusted) %>%
        slice_head(n = 5) # filter for the top 5 protein families
      
      result <- list(EA_tab, Top_protein)
      names(result) <- c("EA_tab", "Top_protein")
      return(result)
...
EAPD_ui_ac <- EACompute(Genepd_ui_ac$APD, Genepd_ui_ac$PD)
head(EAPD_ui_ac$EA_tab)
head(EAPD_ui_ac$Top_protein)
# save the result as a csv file if needed.
# write.csv(as.data.frame(EAPD_ui_ac$EA_tab), "EAPD_Fin_ui_ac.csv", row.names = FALSE)

# add a section to generate bubble graphs for the top 5 PD families.

```

##### Uninfected vs Chronic Controller
```{r perform the enrichment analysis}
EAPD_ui_cc <- EACompute(Genepd_ui_cc$APD, Genepd_ui_cc$PD)
head(EAPD_ui_cc$EA_tab)
head(EAPD_ui_cc$Top_protein)
```

##### Uninfected vs Progressor
```{r perform the enrichment analysis}
EAPD_ui_pr <- EACompute(Genepd_ui_pr$APD, Genepd_ui_pr$PD)
head(EAPD_ui_pr$EA_tab)
head(EAPD_ui_pr$Top_protein)
```

##### Aymptomatic Controller vs Chronic Controller
```{r perform the enrichment analysis}
EAPD_ac_cc <- EACompute(Genepd_ac_cc$APD, Genepd_ac_cc$PD)
head(EAPD_ac_cc$EA_tab)
head(EAPD_ac_cc$Top_protein)
```

##### Aymptomatic Controller vs Progressor
```{r perform the enrichment analysis}
EAPD_ac_pr <- EACompute(Genepd_ac_pr$APD, Genepd_ac_pr$PD)
head(EAPD_ac_pr$EA_tab)
head(EAPD_ac_pr$Top_protein)
```

##### Chronic Controller vs Progressor
```{r perform the enrichment analysis}
EAPD_cc_pr <- EACompute(Genepd_cc_pr$APD, Genepd_cc_pr$PD)
head(EAPD_cc_pr$EA_tab)
head(EAPD_cc_pr$Top_protein)
```
...
### Analysis on biological process
#### Fast analysis using genes and GO terms on org.Mm.eg.db
...
# get data from deg results and from annotation data
deg <- Genego_four$AGO
annotation <- Genego_four$GO


# filter the AGO list for degs with ah alpha level at 0.05, and grab the list of gene id.
gene_list <- Genego_four$AGO[which(Genego_four$AGO[, "adj.P.Val"] > 0), "Gene_id"]
...
tempt <- tempt %>%
  left_join(Genego_four$AGO, by = c("gene_list" = "Gene_id"))
...
# network plot
p_ema <- emapplot(pairwise_termsim(orgdb_result), showCategory = 10, color = "p.adjust", layout.params = list(layout = "nicely"), edge.params = list(min = 0), pie = "Count") 
p_ema
```

#### Manual computation for enrichment analysis
|               At this point, we have both the $\textbf{AGO data}$ from the DEG analysis among the various pairs of contrast groups and the $\textbf{GO data}$ from platform on GEO. Instead of using clusterProfiler, we can manually perform the enrichment/overrepresentation analysis. The theoretical section at the beginning requires computation of the background count $\textbf{m}$, the foreground count $\textbf{q}$, the DEGs of selected GO term $\textbf{x}$, and the number of probes without GO terms $\textbf{n}$. An alpha level of 0.05 is chosen. A different alpha level can be chosen based on the tolerance for Type I and II errors.
...
EACompute <- function(ago, go){
  ...
  if (min(AGO$adj.P.Val) < 0.05){
    ...
    # compute for the counts for relevant terms from the theoretical section for each gene ontology.
    EA <- data.frame() # container for the results of enrichment analysis
    ...
    # check and see if the result dataframe is empty
    if(nrow(EA) > 0 && any(!is.na(EA[, "x"]))){
    # adjust the P.Val.hygeo for multiple testing across the entire array of probes. Finding true positive is more important to me in this analysis, but use one of the other adjusting methods as needed.
      ...
      EA <- cbind(Description = GOname, EA[GOsig, , drop = FALSE]) # ensure dataframe structure not changed.
...
Voilà! Now we have the differentially expressed probes with their protein domains and gene ontology from our microarray! 
